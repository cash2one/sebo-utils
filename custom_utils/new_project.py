...
def assert_webfaction_dir_exists(the_dir):
    ...
    res = vars.webfaction.system(vars.wf_id, "test -d " + vars.webfaction_theme_dir)
    print('RES', res)
    if not res.strip():
    ...
    print(vars.ftp_username,  vars.ftp_password)
...
command = r'pscp -i %UserProfile%\.ssh\private_key.ppk -r {} sebodev@webfaction:{}'.format(vars.project_dir, vars.webfaction_theme_dir)
